...
| 運作流程 | 一個刺激如何變成一個行動。節點座標取自真實網格的分區重心，時間數字全部標了物種、證據型態與爭議程度。 |

另有「自動導覽」16 步，跨檯位講完一輪，空白鍵推進，適合上課投影。

#### 運作流程那一檯的設計

這一檯的形式本身是教學內容，有幾個刻意的限制：

- **零點有四個。** 刺激鎖時、停止訊號鎖時、反應鎖時、口語報告鎖時各自成軌，介面上**一次只能掃一條**。把它們畫在同一條線上，會得出「錯誤偵測（反應後 50–100 ms）比杏仁核分化（刺激後 74 ms）還早」這種荒謬讀法。
- **動物數據自成一軌**，永遠不落在人類軸上。通俗版的「杏仁核 12 毫秒」出自大鼠聽覺系統的單細胞紀錄，搬到人類視覺的時間軸上是錯的。兩軌對不齊本身就是重點。
- **模型反推值不上任何軸。** SSRT 是從賽馬模型反推的，沒有實體零點；純傳導延遲（皮質脊髓 23 ms、底丘腦核誘發電位 2.6 ms）是電纜長度不是歷程。這些放在「不上時間軸的量」那一欄。
- **節點亮起 ≠ 該區必要。** 每個節點另有獨立的「必要性證據」欄位，由病灶研究決定。最好的例子是 Fellows & Farah 2005 的四名背側前扣帶病灶病人，各項控制指標全部正常。
- **階段編號是敘事分區，不是時間順序。** 實測潛伏期與編號順序常常相反——獼猴多巴胺 70–100 ms 遠早於人類內側顳葉 271–394 ms。
- **兩個出口，不是接力。** 杏仁核中央核的防禦輸出不經任何前額葉節點，本身就是一個完成的行動；晚期受控輸出可以攔截它，但會攔不住。

內容經過文獻考證與對抗性查核（2026-09-07），每個時間參數都標了物種、證據型態（單細胞／ERP／顱內／病灶）與爭議程度。
